...
let _missionSetA: IMission[] = undefined;
let _missionSetB: IMission[] = undefined;
...
beforeAll(() => {
  ...
  _missionSetB = _missionSetA.slice(1);
});

test('renders set A of missions', () => {
  const wrapper = shallow(<ApolloViewerReact />);

  wrapper.setState({ missions: _missionSetA });
  expect(wrapper).toMatchSnapshot();

  wrapper.setState({ missions: _missionSetB });
  expect(wrapper).toMatchSnapshot();
});
